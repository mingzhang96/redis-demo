...
#     └── redis.conf
#
# startnodes 启动所有节点 [portNumber]启动对应端口的节点
# startcluster [--localhost]启动本地集群 [--outside]将本地节点添加到已有集群中
# kill [all]关闭所有redis节点 [portNumber]关闭对应端口的节点
# check 检查集群状态
...
# redis的安装路径
redisPath=/usr/local/Cellar/redis/redis-3.2.11/
# 单台机器节点总数
nodes=8
# 从这个端口数开始
startPort=7000
...
if [[ $1 = "create" ]]; then
...
elif [[ $1 = "startnodes" ]] && [[ -n $2 ]]; then
	nohup ${redisPath}/src/redis-server ${rootPath}/${2}/redis.conf >> ${rootPath}/${2}/redis-${2}.log 2>&1 &
	echo "startnode : ${2}"
...
elif [[ $1 = "kill" ]] && [[ $2 = "all" ]]; then
	# centos
	# ret=`ps -fe | grep redis-server | awk '{print $2}' | xargs -i  kill -s 9 {}`
	# macOS
	ret=`ps -ef | grep redis-server | awk '{print $2}' | xargs -I p kill "p"`
	if [[ $ret -ne 0 ]]; then
		echo "kill failed!"
		exit
	else
	...
	# centos
	# ret=`ps -ef | grep "redis-server ${localIP}:$2" | awk '{print $2}' | xargs -i  kill -s 9 {}`
	# macOS
	echo $2
	echo "ps -ef | grep \"redis-server ${localIP}:$2\" | awk '{print \$2}' | xargs -I p kill \"p\""
	ret=`ps -ef | grep "redis-server ${localIP}:$2" | awk '{print $2}' | xargs -I p kill "p"`
	if [[ $ret -ne 0 ]]; then
		echo "kill failed!"
		exit
	else
	...
	echo -e "      <portNumber>      to start portNumber locally"
    ...
    echo -e "  kill"
    echo -e "      <all>             to kill all redis process"
	echo -e "      <portNumber>      to kill portNumber process"
